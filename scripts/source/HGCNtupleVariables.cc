...
void HGCNtupleVariables::getProfile(TProfile*& prof, TH1F* htemp, double LayerEnergy) {
   double temp_total_energy = 0;
   for(int bin = 1; bin <= htemp->GetNbinsX(); bin++) {
     temp_total_energy += htemp->GetBinContent(bin);
     prof->Fill(htemp->GetBinCenter(bin),temp_total_energy/LayerEnergy);
   }
}
